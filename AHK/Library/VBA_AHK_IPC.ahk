...
; 
; Notes:
; On creation, this class creates a memory mapped file (with 
; what ever name you wish) and starts listening to system messages. 
; Once received, the class will read the message that should
; be present on the memory mapped file and run a function with the
; passed function name and arguements. It responds with a "read"
; mesasge and a return value, if one is present.
; *******************************************************
...
Class MemoryMappedFile_IPC
{
	...
	grabNewMessage()
	{
		...
		Loop, 1200 ; = 1 minutes if using 50 msec sleep
		{
			...
			;MsgBox, % V2AMessage . " - " . V2AMessageSplit[1] . " ---- " . V2AMessageSplit[2]
